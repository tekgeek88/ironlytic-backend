...
import (
	...
	"github.com/tekgeek88/ironlytic-backend/internal/api/handlers"
...
func NewRouter(cfg config.Config, logger *slog.Logger, pg *database.Postgres) *gin.Engine {
	...
	// Auth wiring
	userStore := repository.NewUserStore(pg.Pool)
	sessionStore := repository.NewSessionStore(pg.Pool)
